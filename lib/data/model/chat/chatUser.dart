...
class ChatUser {
  ...
  factory ChatUser.fromNotificationData(Map<String, dynamic> json) {
    return ChatUser(
      ...
      translatedName: json["sender_type"] == "0"
          ? "customerSupport"
          : (json['translated_username']?.toString() ?? '').isNotEmpty
              ? json['translated_username']!.toString()
              : (json['username'] ?? '').toString(),
      profile: (json['profile_image'] ?? '').toString(),
      lastMessage: null,
      unReadChats: (json['un_read_chats'] ?? 0).toInt(),
      receiverType: (json['receiver_type'] ?? '').toString(),
      senderType: (json['sender_type'] ?? '').toString(),
      bookingId: (json["booking_id"] ?? json["order_id"] ?? "0").toString(),
      bookingStatus:
          (json["booking_status"] ?? json["order_status"] ?? '').toString(),
      ...
      translatedOrderStatus:
          (json['translated_order_status']?.toString() ?? '').isNotEmpty
              ...
              : (json['booking_status']?.toString() ??
                  json['order_status']?.toString() ??
                  ''),
      providerId: (json["provider_id"] ?? json["partner_id"] ?? "0").toString(),
      senderId: (json["receiver_id"] ?? "0").toString(),
      isBlockedByUser: (json['is_block_by_user'] ?? "0").toString() == "1",
      isBlockedByProvider:
          (json['is_block_by_provider'] ?? "0").toString() == "1",
    );
  }
